...
//	consensusPipelineMinMaxGetCentroid(&cX, &cY);
	...
	centroidDataGet(&cX, &cY, &centroidLite);
	...
	if (pDiff == CONSENSUS_PIPELINE_MINMAX_INV || pMult == CONSENSUS_PIPELINE_MINMAX_INV) {
		oO = 0;
	} else {
		// Scale appropriately
		...
		oO = atan2MilliRad((int32) (2 * pMult), (int32) pDiff) / 2;
	}
...
//	char buffer[100];
//	sprintf(buffer, "%.3f,%.3f\n", centroidLiteGetXCoordinate(), centroidLiteGetYCoordinate());
//	cprintf(buffer);
	...
	/*if (oO > MILLIRAD_HALF_PI) {
		oO -= MILLIRAD_PI;
	} else if (oO < -MILLIRAD_HALF_PI) {
		oO += MILLIRAD_PI;
	}*/


	/*if (cBear>MILLIRAD_PI)
	{
		cBear = cBear - MILLIRAD_2PI;
	}
*/


	int16 ang = sinMilliRad(normalizeAngleMilliRad2(oO - cBear));
	float factor = ((float) ang / (float) MILLIRAD_TRIG_SCALER);

	float w = abs(cDist * factor);


	char outBuffer[500] = {0};	// Output buffer and format
		sprintf(outBuffer, "%.3f,%.3f,%d ,%d,%.3f, %.3f,%.3f\n", cX, cY, oO, cBear,w, cW, cD);
