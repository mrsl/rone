...
/*
 * Called from osTaskList.  vListTasks details all the tasks currently under
...
signed portBASE_TYPE osTaskGenericCreate( pdTASK_CODE pxTaskCode, const signed char * const pcName, unsigned short usStackDepth, void *pvParameters, unsigned portBASE_TYPE uxPriority, xTaskHandle *pxCreatedTask, portSTACK_TYPE *puxStackBuffer, const xMemoryRegion * const xRegions )
{
...
/*-----------------------------------------------------------*/
	...
	void osTaskDelete( xTaskHandle pxTaskToDelete )
	{
	tskTCB *pxTCB;
 ...
 *----------------------------------------------------------*/
	...
	void osTaskDelayUntil( portTickType * const pxPreviousWakeTime, portTickType xTimeIncrement )
	{
	...
	portBASE_TYPE xAlreadyYielded, xShouldDelay = pdFALSE;
		...
		/* Force a reschedule if osTaskResumeAll has not already done so, we may
...
/*-----------------------------------------------------------*/
	...
	void osTaskDelay( portTickType xTicksToDelay )
	{
	portTickType xTimeToWake;
...
/*-----------------------------------------------------------*/
	...
	unsigned portBASE_TYPE osTaskPriorityGet( xTaskHandle pxTask )
	{
	tskTCB *pxTCB;
...
/*-----------------------------------------------------------*/
	...
	void osTaskPrioritySet( xTaskHandle pxTask, unsigned portBASE_TYPE uxNewPriority )
	{
	tskTCB *pxTCB;
...
/*-----------------------------------------------------------*/
	...
	void osTaskSuspend( xTaskHandle pxTaskToSuspend )
	{
	tskTCB *pxTCB;
...
/*-----------------------------------------------------------*/
	...
	signed portBASE_TYPE osTaskIsTaskSuspended( xTaskHandle xTask )
	{
...
/*-----------------------------------------------------------*/
	...
	void osTaskResume( xTaskHandle pxTaskToResume )
	{
	tskTCB *pxTCB;
		...
		if( ( pxTCB != NULL ) && ( pxTCB != pxCurrentTCB ) )
		{
			portENTER_CRITICAL();
			{
				if( osTaskIsTaskSuspended( pxTCB ) == pdTRUE )
				{
...
/*-----------------------------------------------------------*/
	...
	portBASE_TYPE osTaskResumeFromISR( xTaskHandle pxTaskToResume )
	{
...
portBASE_TYPE xReturn;
	...
	//xReturn = osTaskCreate( prvIdleTask, ( signed char * ) "IDLE", tskIDLE_STACK_SIZE, ( void * ) NULL, ( tskIDLE_PRIORITY | portPRIVILEGE_BIT ), ( xTaskHandle * ) pxCreatedTask );
	xReturn = osTaskCreate( prvIdleTask, "IDLE", tskIDLE_STACK_SIZE, ( void * ) NULL, ( tskIDLE_PRIORITY | portPRIVILEGE_BIT ) );
...
signed portBASE_TYPE osTaskResumeAll( void )
{
register tskTCB *pxTCB;
signed portBASE_TYPE xAlreadyYielded = pdFALSE;
...
portTickType osTaskGetTickCount( void )
{
portTickType xTicks;
...
portTickType osTaskGetTickCountFromISR( void )
{
	return xTickCount;
}
/*-----------------------------------------------------------*/

unsigned portBASE_TYPE osTaskGetNumberOfTasks( void )
{
...
/*-----------------------------------------------------------*/
	...
	void osTaskList( signed char *pcWriteBuffer )
	{
...
/*----------------------------------------------------------*/
	...
	void osTaskStartTrace( signed char * pcBuffer, unsigned long ulBufferSize )
	{
		portENTER_CRITICAL();
		{
...
/*----------------------------------------------------------*/
	...
	unsigned long osTaskEndTrace( void )
	{
...
void osTaskPlaceOnEventList( const xList * const pxEventList, portTickType xTicksToWait )
{
portTickType xTimeToWake;
...
signed portBASE_TYPE osTaskRemoveFromEventList( const xList * const pxEventList )
{
...
void osTaskSetTimeOutState( xTimeOutType * const pxTimeOut )
{
...
portBASE_TYPE osTaskCheckForTimeOut( xTimeOutType * const pxTimeOut, portTickType * const pxTicksToWait )
{
portBASE_TYPE xReturn;
	...
	portENTER_CRITICAL();
	{
		...
		else if( ( ( portTickType ) ( ( portTickType ) xTickCount - ( portTickType ) pxTimeOut->xTimeOnEntering ) ) < ( portTickType ) *pxTicksToWait )
		{
			...
			*pxTicksToWait -= ( ( portTickType ) xTickCount - ( portTickType ) pxTimeOut->xTimeOnEntering );
			osTaskSetTimeOutState( pxTimeOut );
			xReturn = pdFALSE;
		}
		else
		{
...
static void prvCheckTasksWaitingTermination( void )
{
	#if ( INCLUDE_vTaskDelete == 1 )
	{
		...
		/* ucTasksDeleted is used to prevent osTaskSuspendAll() being called
...
tskTCB *pxNewTCB;
	...
	if( pxNewTCB != NULL )
	{
		...
		//TODO removed silly word memory reporting
		pxNewTCB->pxStack = ( portSTACK_TYPE * ) pvPortMallocAligned( ( ( ( size_t )usStackDepth ) * sizeof( portSTACK_TYPE ) ), puxStackBuffer );
		//pxNewTCB->pxStack = ( portSTACK_TYPE * ) pvPortMallocAligned( ( ( ( size_t )usStackDepth ) ), puxStackBuffer );
...
/*-----------------------------------------------------------*/
	...
	unsigned portBASE_TYPE osTaskGetStackHighWaterMark( xTaskHandle xTask )
	{
	tskTCB *pxTCB;
...
/*-----------------------------------------------------------*/
	...
	xTaskHandle osTaskGetCurrentTaskHandle( void )
	{
	xTaskHandle xReturn;
...
/*-----------------------------------------------------------*/
	...
	void osTaskPriorityInherit( xTaskHandle * const pxMutexHolder )
	{
...
/*-----------------------------------------------------------*/
	...
	void osTaskPriorityDisinherit( xTaskHandle * const pxMutexHolder )
	{
	tskTCB * const pxTCB = ( tskTCB * ) pxMutexHolder;
...
const char* osTaskGetName( xTaskHandle xTask ) {
	...
	pxTCB = prvGetTCBFromHandle( xTask );
	return (const char*)pxTCB->pcTaskName;
}

xTaskHandle sysTaskHandles[SYS_MAX_TASK_NUM];
unsigned short sysTaskStackSizes[SYS_MAX_TASK_NUM];
unsigned char sysTaskCount = 0;
void cprintf(char *format, ...);

signed portBASE_TYPE osTaskCreate(pdTASK_CODE pxTaskCode, const char * const pcName,
		unsigned short usStackDepth, void *pvParameters, unsigned portBASE_TYPE uxPriority) {
	xTaskHandle taskHandle;
	portBASE_TYPE val;

	if (sysTaskCount < SYS_MAX_TASK_NUM) {
		//TODO silly word memory hack.  divide, then multiply to clear low-order bits and align
		usStackDepth = usStackDepth/sizeof( portSTACK_TYPE );
		val = osTaskGenericCreate(pxTaskCode, (const signed char * const)pcName, usStackDepth, pvParameters, uxPriority, &taskHandle, NULL, NULL);

		if(val == pdTRUE) {
			// save the task handle
			sysTaskHandles[sysTaskCount] = taskHandle;
			sysTaskStackSizes[sysTaskCount++] = usStackDepth * sizeof( portSTACK_TYPE );
		}else {
			// error. print a message
			cprintf("FreeRTOS error %d.  Could not create task \"%s\"\n", val, pcName);
		}
	} else {
		// error. print a message
//		cprintf("too many tasks, max %d.  Could not create task %s", SYS_MAX_TASK_NUM, pcName);
		val = errCOULD_NOT_ALLOCATE_REQUIRED_MEMORY;
	}
	return val;
}
