...
void rprintf(const char *format, ...) {
	...
	maxLength = RPRINTF_TEXT_STRING_SIZE - rprintfWriteBufferLength - 1;
	...
	// Check for overflow
	if ((rprintfWriteBufferLength += n) >= maxLength) {
		error("nrprintf buffer overflow");
		rprintfWriteBufferLength = maxLength;
	}
